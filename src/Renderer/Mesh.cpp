...
bool Mesh::IntersectLeaf(const NodeSerialized& N, const Ray& R, TriangleIntersection& IntersectionInfo) const {
	...
	for (auto Iter = Begin; Iter != End; Iter++) {
		...
		Hit |= Triangles[*Iter].Intersect(R, IntersectionInfo);
	}

	return Hit;
}

bool Mesh::Intersect(const Ray& R, TriangleIntersection& IntersectionInfo) const {
	bool SuccessfulIntersection = false;

	Ray InverseRay;

	InverseRay.Direction = 1.0f / R.Direction;
	InverseRay.Origin = -R.Origin * InverseRay.Direction;

	AABBIntersection RootNodeIntersection;
	RootNodeIntersection.Depth = IntersectionInfo.Depth;
	if (!BVH.Nodes[0].BoundingBox.Intersect(InverseRay, RootNodeIntersection)) {
		return false;
	}
	...
	while (true) {
		...
		bool AABBHit[2] = {
			Children[0].BoundingBox.Intersect(InverseRay, IntersectionBounds[0]),
			Children[1].BoundingBox.Intersect(InverseRay, IntersectionBounds[1]),
		};
		...
		if (AABBHit[0] && AABBHit[1]) {
			...
			continue;
		} else if (AABBHit[0] && !AABBHit[1]) {
			CurrentIndex = Children[0].ChildrenNodes[0];
			continue;
		} else if (AABBHit[1] && !AABBHit[0]) {
			CurrentIndex = Children[1].ChildrenNodes[0];
			continue;
		} else {
			if (Stack.Nonempty()) {
